...
export default function FrameworkManagement() {
  ...
  const handleCreateFramework = async (newFramework: Omit<Framework, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      const created = await createFramework.mutateAsync(newFramework);
  ...
  // Create new section
  const handleCreateSection = async (newSection: Omit<FrameworkSection, 'id'>) => {
    if (!selectedFramework) return;

    const sectionWithId: FrameworkSection = {
      ...newSection,
      id: `section-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    };

    const updatedSections = [...selectedFramework.sections, sectionWithId];
    ...
    try {
      await updateFramework.mutateAsync({
        ...
        data: {
          ...
          totalQuestions: selectedFramework.totalQuestions + (sectionWithId.questions?.length || 0),
          requiredQuestions: selectedFramework.requiredQuestions + (sectionWithId.questions?.filter(q => q.isRequired).length || 0)
        }
      });
      setSelectedFramework({ ...selectedFramework, sections: updatedSections });
  ...
  const handleDeleteSection = async (sectionIdx: number) => {
    ...
    const section = selectedFramework.sections[sectionIdx];
    if (!confirm(`Delete section "${section.title}"? This will also delete ${section.questions?.length || 0} questions.`)) {
      return;
    }

    const updatedSections = selectedFramework.sections.filter((_, idx) => idx !== sectionIdx);
    const deletedQuestions = section.questions?.length || 0;
    const deletedRequiredQuestions = section.questions?.filter(q => q.isRequired).length || 0;
    ...
    try {
      await updateFramework.mutateAsync({
        ...
        data: {
          sections: updatedSections,
          totalQuestions: selectedFramework.totalQuestions - deletedQuestions,
          requiredQuestions: selectedFramework.requiredQuestions - deletedRequiredQuestions
        }
      });
      setSelectedFramework({ ...selectedFramework, sections: updatedSections });
    } catch (err) {
  ...
  // Create new question
  const handleCreateQuestion = async (newQuestion: Omit<Question, 'id'>, sectionIdx: number) => {
    if (!selectedFramework) return;
    ...
    const questions = [...(section.questions || []), questionWithId];
    section.questions = questions;
    ...
    try {
      await updateFramework.mutateAsync({
        ...
        data: {
          sections: updatedSections,
          totalQuestions: selectedFramework.totalQuestions + 1,
          requiredQuestions: selectedFramework.requiredQuestions + (questionWithId.isRequired ? 1 : 0)
        }
      });
      setSelectedFramework({ ...selectedFramework, sections: updatedSections });
  ...
  const handleDeleteQuestion = async (sectionIdx: number, questionIdx: number) => {
    ...
    const question = selectedFramework.sections[sectionIdx].questions[questionIdx];
    ...
    const updatedSections = [...selectedFramework.sections];
    const section = { ...updatedSections[sectionIdx] };
    const questions = section.questions.filter((_, idx) => idx !== questionIdx);
    section.questions = questions;
    updatedSections[sectionIdx] = section;
    ...
    try {
      await updateFramework.mutateAsync({
        ...
        data: {
          sections: updatedSections,
          totalQuestions: selectedFramework.totalQuestions - 1,
          requiredQuestions: selectedFramework.requiredQuestions - (question.isRequired ? 1 : 0)
        }
      });
      setSelectedFramework({ ...selectedFramework, sections: updatedSections });
    } catch (err) {
  ...
  const handleDeleteFramework = async (id: string, name: string) => {
    if (!confirm(`Delete framework "${name}"? This action cannot be undone.`)) {
      return;
    }
  ...
  const openCreateQuestion = (sectionIdx: number) => {
    setEditingSectionIdx(sectionIdx);
  ...
  const getAlignmentColorClasses = (color: string) => {
    const colorMap: Record<string, string> = {
      ...
      blue: 'bg-sas-navy-100 text-blue-800 border-sas-navy-200',
  ...
  const columns: Column<Framework>[] = [
    ...
    },
    {
      ...
      header: 'Version',
      accessor: (framework) => (
        <span className="px-3 py-1 text-sm font-medium bg-sas-navy-100 text-blue-800 rounded-full">
          v{framework.version}
        </span>
      ),
      width: 'w-24'
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">
            Create, edit, and manage observation frameworks • Connected to Firestore
          </p>
        </div>
        <button
                ...
                <div key={section.id} className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-50">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-2 py-0.5 text-xs font-medium bg-sas-navy-100 text-blue-800 rounded">
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600">
                      <span className="font-medium mr-2">{section.questions?.length || 0} questions</span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                  ...
                  {expandedSections.has(section.id) && (
                    <div className="p-6 bg-white border-t border-gray-200">
                      ...
                      <div className="mb-4">
                        <button
                          onClick={() => openCreateQuestion(sectionIdx)}
                        ...
                        section.questions.map((question, idx) => (
                          ...
                          >
                            ...
                            <div className="absolute top-4 right-4 flex items-center gap-2">
                              ...
                              <button
                                onClick={() => handleDeleteQuestion(sectionIdx, idx)}
                                className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors"
                            ...
                            <div className="flex items-start gap-3 mb-3 pr-12">
                              <span className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-sas-navy-600 text-white rounded-full text-sm font-bold">
                            ...
                            {/* Help Text */}
                            {question.helpText && (
                              <div className="ml-11 mb-3 p-3 bg-sas-navy-50 border border-blue-100 rounded-lg">
                                <p className="text-sm text-sas-navy-900">
                                ...
                                <ul className="space-y-1">
                                  {question.examples.map((example, exIdx) => (
                                    <li key={exIdx} className="text-sm text-gray-600 flex items-start gap-2">
                                      <span className="text-sas-navy-600 font-bold">•</span>
                                      <span>{example}</span>
                                    </li>
                                  ))}
                                </ul>
      ...
      {isCreatingFramework && user && (
        <CreateFrameworkModal
          user={user}
          onSave={handleCreateFramework}
          onClose={() => setIsCreatingFramework(false)}
        />
      )}
      ...
      {isCreatingSection && (
        <CreateSectionModal
          ...
          onClose={() => setIsCreatingSection(false)}
        />
      )}

      {/* Create Question Modal */}
      {isCreatingQuestion && selectedFramework && editingSectionIdx !== null && (
        <CreateQuestionModal
