...
export const useObservations = (schoolId) => {
...
export const useObservation = (observationId) => {
    return useQuery({
...
export const useDeleteObservation = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => observationApi.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['observations'] });
        },
    });
};
export const useObservationSubscription = (observationId, callback) => {
    return useQuery({
