...
export const useFramework = (frameworkId) => {
    return useQuery({
...
export const useCreateFramework = () => {
    ...
    return useMutation({
        mutationFn: (data) => frameworkApi.create(data),
        onSuccess: () => {
...
export const useUpdateFramework = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, data }) => frameworkApi.update(id, data),
        onSuccess: (_, { id }) => {
            queryClient.invalidateQueries({ queryKey: ['frameworks'] });
...
export const useDeleteFramework = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => frameworkApi.delete(id),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['frameworks'] });
        },
    });
