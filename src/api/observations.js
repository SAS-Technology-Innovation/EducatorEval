...
import { collection, doc, getDocs, getDoc, addDoc, updateDoc, deleteDoc, query, where, orderBy, limit as firestoreLimit, Timestamp } from 'firebase/firestore';
...
export const observationsApi = {
    ...
    observations: {
        list: async (filters) => {
            ...
            if (filters?.dateFrom && filters?.dateTo) {
                q = query(q, where('observationDate', '>=', Timestamp.fromDate(filters.dateFrom)), where('observationDate', '<=', Timestamp.fromDate(filters.dateTo)));
            }
            if (filters?.limit) {
        ...
        autoPopulateFromSchedule: async (educatorId, date) => {
            try {
    ...
    analytics: {
        ...
        exportObservations: async (format, filters) => {
            const token = await getAuthToken();
